...
var memoize = function (fn) {
  ...
  var memoizedFn = function (obj, path, fallback, index) {
...
var fromEntries = function (entries) { return entries.reduce(function (carry, _ref) {
  var key = _ref[0];
  var value = _ref[1];
  carry[key] = value;
  return carry;
...
function px(value) {
  ...
  var ref = analyzeCSSValue(value);
  var unitless = ref.unitless;
  return unitless || isNumber(value) ? value + "px" : value;
}

var sortByBreakpointValue = function (a, b) { return parseInt(a[1], 10) > parseInt(b[1], 10) ? 1 : -1; };

var sortBps = function (breakpoints) { return fromEntries(Object.entries(breakpoints).sort(sortByBreakpointValue)); };
...
function subtract(value) {
  ...
  -0.0635;
  return isNumber(value) ? "" + (value + factor) : value.replace(/([0-9]+\.?[0-9]*)/, function (m) { return "" + (parseFloat(m) + factor); });
}
...
function analyzeBreakpoints(breakpoints) {
  ...
  var queries = Object.entries(breakpoints).sort(sortByBreakpointValue).map(function (_ref, index, entry) {
    ...
    var minW = _ref[1];
    var ref = (_entry = entry[index + 1]) != null ? _entry : [];
    var maxW = ref[1];
  ...
  return {
    ...
    isResponsive: function isResponsive(test) {
      ...
      return keys.length > 0 && keys.every(function (key) { return _keys.has(key); });
    },

    asObject: sortBps(breakpoints),
    asArray: normalize(breakpoints),
    details: queries,
    media: [null ].concat( normalized.map(function (minW) { return queryString(minW); }).slice(1)),
...
function walkObject(target, predicate) {
  function inner(value, path) {
    ...
    if (isArray(value)) {
      return value.map(function (item, index) { return inner(item, path.concat( [String(index)])); });
    }
    ...
    if (isObject(value)) {
      return Object.fromEntries(Object.entries(value).map(function (_ref) {
        var key = _ref[0];
        var child = _ref[1];
        return [key, inner(child, path.concat( [key]))];
      }));
    }
...
var tokenToCSSVar = function (scale, value) { return function (theme) {
...
var getRtl = function (_ref) {
  var rtl = _ref.rtl;
  var ltr = _ref.ltr;
  return function (theme) { return theme.direction === "rtl" ? rtl : ltr; };
};
...
var spaceXTemplate = {};
spaceXTemplate[owlSelector] = {
    marginInlineStart: "calc(var(--chakra-space-x) * calc(1 - var(--chakra-space-x-reverse)))",
    marginInlineEnd: "calc(var(--chakra-space-x) * var(--chakra-space-x-reverse))"
  };
var spaceYTemplate = {};
spaceYTemplate[owlSelector] = {
...
var trimSpace = function (str) { return str.trim(); };
...
function parseGradient(value, theme) {
  ...
  var ref = (_regex$exec$groups = (_regex$exec = regex.exec(value)) == null ? void 0 : _regex$exec.groups) != null ? _regex$exec$groups : {};
  var type = ref.type;
  ...
  var ref$1 = values.split(",").map(trimSpace).filter(Boolean);
  var maybeDirection = ref$1[0];
  var stops = ref$1.slice(1);
  ...
  var _values = stops.map(function (stop) {
    ...
    var ref = firstStop !== -1 ? [stop.substr(0, firstStop), stop.substr(firstStop + 1)] : [stop];
    var _color = ref[0];
    var _stop = ref[1];
...
var isCSSFunction = function (value) {
  return isString(value) && value.includes("(") && value.includes(")");
};
var gradientTransform = function (value, theme) { return parseGradient(value, theme != null ? theme : {}); };

var analyzeCSSValue$1 = function (value) {
  var num = parseFloat(value.toString());
  var unit = value.toString().replace(String(num), "");
  return {
    unitless: !unit,
...
var wrap = function (str) { return function (value) { return str + "(" + value + ")"; }; };
...
var transformFunctions = {
  ...
  px: function px(value) {
    if (value == null) { return value; }
    var ref = analyzeCSSValue$1(value);
    var unitless = ref.unitless;
    return unitless || isNumber(value) ? value + "px" : value;
  },
  ...
  flexDirection: function flexDirection(value) {
    ...
    var ref = (_flexDirectionTemplat = flexDirectionTemplate[value]) != null ? _flexDirectionTemplat : {};
    var space = ref.space;
    var divide = ref.divide;
    var result = {
      flexDirection: value
    };
...
var flexbox = {
  ...
  experimental_spaceX: {
    ...
    transform: createTransform({
      scale: "space",
      transform: function (value) { return value !== null ? {
...
var getWithPriority = function (theme, key, styles) {
  var result = {};
...
var others = {
  ...
  layerStyle: {
    processResult: true,
    transform: function (value, theme, styles) { return getWithPriority(theme, "layerStyles." + value, styles); }
  },
  textStyle: {
    processResult: true,
    transform: function (value, theme, styles) { return getWithPriority(theme, "textStyles." + value, styles); }
  },
  apply: {
    processResult: true,
    transform: function (value, theme, styles) { return getWithPriority(theme, value, styles); }
  }
};
...
var group = {
  ...
  focus: function (selector) { return selector + ":focus &, " + selector + "[data-focus] &"; },
  active: function (selector) { return selector + ":active &, " + selector + "[data-active] &"; },
  disabled: function (selector) { return selector + ":disabled &, " + selector + "[data-disabled] &"; },
  invalid: function (selector) { return selector + ":invalid &, " + selector + "[data-invalid] &"; },
  checked: function (selector) { return selector + ":checked &, " + selector + "[data-checked] &"; },
  indeterminate: function (selector) { return selector + ":indeterminate &, " + selector + "[aria-checked=mixed] &, " + selector + "[data-indeterminate] &"; },
  readOnly: function (selector) { return selector + ":read-only &, " + selector + "[readonly] &, " + selector + "[data-read-only] &"; },
  expanded: function (selector) { return selector + ":read-only &, " + selector + "[aria-expanded=true] &, " + selector + "[data-expanded] &"; }
};

var toGroup = function (fn) { return merge(fn, "[role=group]", "[data-group]", ".group"); };
...
function _extends$1() { _extends$1 = Object.assign || function (target) {
var arguments$1 = arguments;
 for (var i = 1; i < arguments.length; i++) { var source = arguments$1[i]; for (var key in source) { if (Object.prototype.hasOwnProperty.call(source, key)) { target[key] = source[key]; } } } return target; }; return _extends$1.apply(this, arguments); }
...
var propNames = objectKeys(systemProps).concat( pseudoPropNames);

var styleProps = _extends$1({}, systemProps, pseudoSelectors);

var isStyleProp = function (prop) { return prop in styleProps; };
...
var expandResponsive = function (styles) { return function (theme) {
  /**
  ...
  var ref = theme.__breakpoints;
  var isResponsive = ref.isResponsive;
  var toArrayValue = ref.toArrayValue;
  var medias = ref.media;
  var computedStyles = {};
...
var isCSSVariableTokenValue = function (key, value) { return key.startsWith("--") && isString(value) && !isCssVar(value); };

var resolveTokenValue = function (theme, value) {
  var _ref, _getVar2;
  ...
  var valueSplit = value.split(",").map(function (v) { return v.trim(); });
  var tokenValue = valueSplit[0];
  var fallbackValue = valueSplit[1];
  value = (_ref = (_getVar2 = getVar(tokenValue)) != null ? _getVar2 : getValue(fallbackValue)) != null ? _ref : getValue(value);
  return value;
};

function getCss(options) {
  var configs = options.configs; if ( configs === void 0 ) configs = {};
  var pseudos = options.pseudos; if ( pseudos === void 0 ) pseudos = {};
  ...
  var css = function css(stylesOrFn, nested) {
    ...
    for (var key in styles) {
      ...
      if (configProperty && Array.isArray(configProperty)) {
        for (var i = 0, list = configProperty; i < list.length; i += 1) {
          var property = list[i];
...
var css = function (styles) { return function (theme) {
  var cssFn = getCss({
    theme: theme,
...
var calc = Object.assign(function (x) { return ({
  add: function add() {
    var arguments$1 = arguments;
  ...
  negate: function () { return calc(_negate(x)); },
  toString: function () { return x.toString(); }
}); }, {
  add: _add,
  subtract: _subtract,
  multiply: _multiply,
...
function _extends$2() { _extends$2 = Object.assign || function (target) {
var arguments$1 = arguments;
 for (var i = 1; i < arguments.length; i++) { var source = arguments$1[i]; for (var key in source) { if (Object.prototype.hasOwnProperty.call(source, key)) { target[key] = source[key]; } } } return target; }; return _extends$2.apply(this, arguments); }
function createThemeVars(target, options) {
  ...
  walkObject(target, function (value, path) {
    ...
    var ref = handler(path, value, options);
    var cssVars = ref.cssVars;
    var cssMap = ref.cssMap;
 ...
 */
var tokenHandlerMap = {
  space: function (keys, value, options) {
    var obj;

    var properties = tokenHandlerMap.defaultHandler(keys, value, options);
    var firstKey = keys[0];
    var referenceKeys = keys.slice(1);
    var negativeLookupKey = firstKey + ".-" + referenceKeys.join(".");
    var negativeVarKey = keys.join("-");
    var ref = cssVar(negativeVarKey, undefined, options.cssVarPrefix);
    ...
    return {
      ...
      cssMap: _extends$2({}, properties.cssMap, ( obj = {}, obj[negativeLookupKey] = {
          value: "" + negativeValue,
          var: "" + variable,
          varRef: varRef
        }, obj ))
  ...
  defaultHandler: function (keys, value, options) {
    var obj, obj$1;

    var lookupKey = keys.join(".");
    var varKey = keys.join("-");
    var ref = cssVar(varKey, undefined, options.cssVarPrefix);
    var variable = ref.variable;
    var reference = ref.reference;
    return {
      cssVars: ( obj = {}, obj[variable] = value, obj ),
      cssMap: ( obj$1 = {}, obj$1[lookupKey] = {
          value: value,
          var: variable,
          varRef: reference
        }, obj$1 )
...
function _extends$3() { _extends$3 = Object.assign || function (target) {
var arguments$1 = arguments;
 for (var i = 1; i < arguments.length; i++) { var source = arguments$1[i]; for (var key in source) { if (Object.prototype.hasOwnProperty.call(source, key)) { target[key] = source[key]; } } } return target; }; return _extends$3.apply(this, arguments); }
function toCSSVar(rawTheme) {
  ...
  var ref = createThemeVars(tokens, {
